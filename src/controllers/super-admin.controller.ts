...
export const getDashboardData = async (req: Request, res: Response) => {
  try {
    ...
    const companies = Array.isArray(companiesResult) ? Number((companiesResult[0] as any)?.count || 0) : 0;
    ...
    const monthlyRevenue = Array.isArray(revenueResult) ? Number((revenueResult[0] as any)?.total || 0) : 0;
...
export const getKPIMetrics = async (req: Request, res: Response) => {
  try {
    ...
    const revenue = Array.isArray(totalRevenue) ? Number((totalRevenue[0] as any)?.total || 0) : 0;
...
export const getAuditLogs = async (req: Request, res: Response) => {
  try {
    ...
    const total = Array.isArray(totalResult) ? Number((totalResult[0] as any)?.count || 0) : 0;
...
export const getPlatformAnalytics = async (req: Request, res: Response) => {
  try {
    ...
    const properties = Array.isArray(totalProperties) ? Number((totalProperties[0] as any)?.count || 0) : 0;
    const units = Array.isArray(totalUnits) ? Number((totalUnits[0] as any)?.count || 0) : 0;
    const revenue = Array.isArray(totalRevenue) ? Number((totalRevenue[0] as any)?.total || 0) : 0;
...
export const getRevenueDashboard = async (req: Request, res: Response) => {
  try {
    ...
    const total = Array.isArray(totalRevenue) ? Number((totalRevenue[0] as any)?.total || 0) : 0;
    const monthly = Array.isArray(monthlyRevenue) ? Number((monthlyRevenue[0] as any)?.monthly || 0) : 0;
...
export const getRevenueSummary = async (req: Request, res: Response) => {
  try {
    ...
    const current = Array.isArray(currentRevenue) ? Number((currentRevenue[0] as any)?.total || 0) : 0;
    const previous = Array.isArray(previousRevenue) ? Number((previousRevenue[0] as any)?.total || 0) * 0.9 : 0; // Mock 10% growth
