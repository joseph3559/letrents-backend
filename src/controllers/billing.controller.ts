...
export const verifySubscription = async (req: Request, res: Response) => {
  try {
    let user = (req as any).user as JWTClaims | undefined;
    ...
    if (!finalPlanCode) {
      ...
      if (existingSub && existingSub.paystack_plan_code) {
        ...
        const isAgency = user.role === 'agency_admin' || user.role === 'agent';
        const planCodeToName: { [key: string]: { landlord: string; agency: string } } = {
        ...
        await prisma.subscription.update({
          ...
          data: {
            status: 'active',
            metadata: {
              ...(existingSub.metadata as object || {}),
    ...
    const planConfig = (paystackServiceInstance as any).plans[planName];
