 ...
 */
export const getCompanySubaccount = async (req: Request, res: Response) => {
  try {
    const user = (req as any).user as JWTClaims;

    if (!user?.company_id) return writeError(res, 400, 'User must belong to a company');
    ...
    if (!company) return writeError(res, 404, 'Company not found');
 ...
 */
export const upsertCompanySubaccount = async (req: Request, res: Response) => {
  try {
    const user = (req as any).user as JWTClaims;
 ...
 */
export const resolvePaystackAccount = async (req: Request, res: Response) => {
  try {
    const user = (req as any).user as JWTClaims;
    if (!['landlord', 'agency_admin', 'super_admin'].includes(user.role)) {
    ...
    if (!account_number || !bank_code) return writeError(res, 400, 'account_number and bank_code are required');
 ...
 */
export const getRentRoutingContext = async (req: Request, res: Response) => {
  try {
    const user = (req as any).user as JWTClaims;
    if (user.role !== 'tenant') return writeError(res, 403, 'Tenant role required');
    ...
    if (invoices.length === 0) return writeError(res, 404, 'No invoices found for tenant');
    const companyIds = Array.from(new Set(invoices.map((i: any) => i.company_id)));
    ...
    const rentAmountKES = invoices.reduce((s: number, inv: any) => s + Number(inv.total_amount || 0), 0);
