...
export class ChecklistsController {
  ...
  getTemplates = async (req: Request, res: Response): Promise<void> => {
    try {
      const user = req.user as JWTClaims;
      const filters = {
        inspection_type: req.query.inspection_type as any,
        property_id: req.query.property_id as string,
        is_active: req.query.is_active === 'true' ? true : req.query.is_active === 'false' ? false : undefined,
        agencyId: req.query.agencyId as string, // For super-admin viewing as specific agency
      };
   ...
   */
  recordInspectionItem = async (req: Request, res: Response): Promise<void> => {
    try {
      const user = req.user as JWTClaims;
   ...
   */
  uploadInspectionPhoto = async (req: Request, res: Response): Promise<void> => {
    try {
      const user = req.user as JWTClaims;
