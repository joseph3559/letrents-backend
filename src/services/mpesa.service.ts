...
export interface C2BTransactionData {
...
export interface PaybillSettingsRequest {
...
export class MpesaService {
  ...
  private accessTokenCache: { token: string; expires: number } | null = null;
   ...
   */
  async registerC2BURL(credentials: MpesaCredentials, request: C2BRegisterRequest): Promise<any> {
    try {
   ...
   */
  async createPaybillSettings(request: PaybillSettingsRequest, user: JWTClaims) {
    // Check permissions
   ...
   */
  async getPaybillSettings(user: JWTClaims) {
    if (!user.company_id) {
   ...
   */
  async validateC2BTransaction(data: C2BTransactionData): Promise<{ ResultCode: number; ResultDesc: string }> {
    try {
   ...
   */
  async confirmC2BTransaction(data: C2BTransactionData): Promise<{ ResultCode: number; ResultDesc: string }> {
    try {
   ...
   */
  async reconcileTransaction(transactionId: string, user?: JWTClaims) {
   ...
   */
  async getTransactions(user: JWTClaims, page = 1, limit = 10, filters: any = {}) {
   ...
   */
  async getTransactionStats(user: JWTClaims, period = 'monthly') {
    const whereClause = buildWhereClause(user, {}, 'mpesa');
