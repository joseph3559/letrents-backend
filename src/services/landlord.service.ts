...
export const landlordService = {
  // Dashboard services
  getDashboard: async (user: JWTClaims) => {
  ...
  getDashboardStats: async (user: JWTClaims) => {
    return await landlordService.getDashboard(user);
  },

  // Properties services
  getProperties: async (user: JWTClaims, filters: any) => {
    ...
    const whereClause: any = { owner_id: user.user_id };
  ...
  // Units services
  getUnits: async (user: JWTClaims, filters: any) => {
    const whereClause: any = user.company_id ? { company_id: user.company_id } : {};
  ...
  // Tenants services
  getTenants: async (user: JWTClaims, filters: any) => {
    ...
    if (directTenantIds.length === 0 && propertyIds.length > 0) {
      ...
      const leaseTenantIds = [...new Set(leases.map((l: any) => l.tenant_id))];
      ...
      const leaseTenantIds = leases.map((l: any) => l.tenant_id);
  ...
  // Financial services
  getFinancialOverview: async (user: JWTClaims) => {
    const whereClause = user.company_id ? { company_id: user.company_id } : {};
  ...
  getPaymentHistory: async (user: JWTClaims, filters: any) => {
  ...
  getRentCollectionStats: async (user: JWTClaims, period: string) => {
    const whereClause = user.company_id ? { company_id: user.company_id } : {};
  ...
  getMaintenanceOverview: async (user: JWTClaims) => {
    const whereClause = user.company_id ? { company_id: user.company_id } : {};
  ...
  getMaintenanceRequests: async (user: JWTClaims, filters: any) => {
    const whereClause: any = user.company_id ? { company_id: user.company_id } : {};
  ...
  getInspectionOverview: async (user: JWTClaims) => {
    return {
      total_inspections: 0,
  ...
  getInspectionSchedule: async (user: JWTClaims, filters: any) => {
    return {
      inspections: [],
      total: 0,
    };
  ...
  getCommunicationOverview: async (user: JWTClaims) => {
    return {
      total_messages: 0,
      unread_messages: 0,
      sent_messages: 0,
  ...
  getMessages: async (user: JWTClaims, filters: any) => {
    return {
      messages: [],
      total: 0,
    };
  ...
  // Reports services
  getReports: async (user: JWTClaims, reportType?: string, period: string = 'monthly') => {
    const whereClause = user.company_id ? { company_id: user.company_id } : {};
  ...
  generatePropertyReport: async (user: JWTClaims, params: any) => {
    return await landlordService.getReports(user, 'property', params.period);
  },

  generateFinancialReport: async (user: JWTClaims, params: any) => {
    return await landlordService.getReports(user, 'financial', params.period);
  },

  generateOccupancyReport: async (user: JWTClaims, period: string) => {
    return await landlordService.getReports(user, 'occupancy', period);
  },

  getRentCollectionDetails: async (user: JWTClaims, filters: any) => {
    return await landlordService.getRentCollectionStats(user, filters.period || 'monthly');
  },

  // Notifications services (placeholder)
  getNotifications: async (user: JWTClaims, limit: number, offset: number, category?: string, status?: string) => {
    const whereClause: any = user.company_id ? { company_id: user.company_id } : {};
  ...
  markNotificationAsRead: async (user: JWTClaims, notificationId: string) => {
  ...
  getUnreadNotificationCount: async (user: JWTClaims) => {
    const whereClause: any = {
      is_read: false,
  ...
  markAllNotificationsAsRead: async (user: JWTClaims) => {
    const whereClause: any = {
      is_read: false,
      recipient_id: user.user_id,
    };
  ...
  deleteNotification: async (user: JWTClaims, notificationId: string) => {
  ...
  createCaretaker: async (user: JWTClaims, caretakerData: any) => {
  ...
  getCaretakerDetails: async (user: JWTClaims, caretakerId: string) => {
  ...
  updateCaretaker: async (user: JWTClaims, caretakerId: string, updateData: any) => {
  ...
  deleteCaretaker: async (user: JWTClaims, caretakerId: string) => {
  ...
  getInvoices: async (user: JWTClaims, filters: any) => {
    const whereClause: any = user.company_id ? { company_id: user.company_id } : {};
