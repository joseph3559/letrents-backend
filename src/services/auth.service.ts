...
export class AuthService {
	...
	private generateJwt(user: any, sessionId: string, permissions: string[] = []): { token: string; expiresAt: Date } {
	...
	private async createRefreshToken(userId: string, deviceInfo?: unknown, ip?: string, ua?: string, hours?: number) {
		...
		await this.prisma.refreshToken.create({
			data: {
				...
				device_info: deviceInfo ? (deviceInfo as any) : undefined,
	...
	async verifyEmail(token: string): Promise<void> {
		...
		if (rec.expires_at && rec.expires_at < new Date()) throw new Error('verification token is expired');
	...
	async login(payload: { email: string; password: string; remember_me?: boolean; device_info?: unknown }, ip?: string, ua?: string) {
	...
	async refresh(refreshToken: string, ip?: string, ua?: string) {
		...
		if (!rec || rec.is_revoked) throw new Error('invalid token');
	...
	async resetPassword(token: string, newPassword: string): Promise<void> {
		...
		if (rec.expires_at && rec.expires_at < new Date()) throw new Error('reset token is expired');
