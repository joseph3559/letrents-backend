...
function mapUnitType(frontendType: string): string {
...
function mapUtilityBillingType(frontendType: string): string {
...
function mapUnitCondition(frontendCondition: string): string {
...
export class UnitsService {
  ...
  async createUnit(req: CreateUnitRequest, user: JWTClaims): Promise<any> {
    ...
    const unitData = {
      property_id: req.property_id,
      unit_number: req.unit_number,
      unit_type: mapUnitType(req.unit_type) as any,
      ...
      condition: mapUnitCondition(req.condition || 'good') as any,
      furnishing_type: (req.furnishing_type || 'unfurnished') as any,
      water_meter_number: req.water_meter_number,
      electric_meter_number: req.electric_meter_number,
      utility_billing_type: mapUtilityBillingType(req.utility_billing_type || 'postpaid') as any,
  ...
  async createUnits(req: CreateUnitsRequest, user: JWTClaims): Promise<any[]> {
    // Validate user permissions
    if (!['super_admin', 'agency_admin', 'landlord'].includes(user.role)) {
      throw new Error('insufficient permissions to create units');
    }
  ...
  async getUnit(id: string, user: JWTClaims): Promise<any> {
    ...
    if (unit.leases.length > 0) {
      ...
      (unit as any).active_lease = activeLease;
  ...
  async getUnitFinancials(id: string, user: JWTClaims): Promise<any> {
  ...
  async updateUnit(id: string, req: UpdateUnitRequest, user: JWTClaims): Promise<any> {
    ...
    const unit = await this.prisma.unit.update({
      ...
      data: {
        ...
        ...(req.unit_type && { unit_type: req.unit_type as any }),
        ...
        ...(req.condition && { condition: mapUnitCondition(req.condition) as any }),
        ...(req.furnishing_type && { furnishing_type: req.furnishing_type as any }),
  ...
  async deleteUnit(id: string, user: JWTClaims): Promise<void> {
    // First check if unit exists and user has access
    const existingUnit = await this.getUnit(id, user);
  ...
  async listUnits(filters: UnitFilters, user: JWTClaims): Promise<any> {
    ...
    if (filters.furnishing_type) where.furnishing_type = filters.furnishing_type;
    if (filters.current_tenant_id) where.current_tenant_id = filters.current_tenant_id;
    if (filters.block_number) where.block_number = { contains: filters.block_number, mode: 'insensitive' };
    if (filters.floor_number !== undefined) where.floor_number = filters.floor_number;
    if (filters.lease_type) where.lease_type = filters.lease_type;

    // Boolean filters
    if (filters.has_ensuite !== undefined) where.has_ensuite = filters.has_ensuite;
    if (filters.has_balcony !== undefined) where.has_balcony = filters.has_balcony;
    if (filters.has_parking !== undefined) where.has_parking = filters.has_parking;

    // Range filters
    if (filters.min_rent || filters.max_rent) {
      where.rent_amount = {};
      if (filters.min_rent) where.rent_amount.gte = filters.min_rent;
      if (filters.max_rent) where.rent_amount.lte = filters.max_rent;
    }

    if (filters.min_bedrooms || filters.max_bedrooms) {
      where.number_of_bedrooms = {};
      if (filters.min_bedrooms) where.number_of_bedrooms.gte = filters.min_bedrooms;
      if (filters.max_bedrooms) where.number_of_bedrooms.lte = filters.max_bedrooms;
    }

    if (filters.min_bathrooms || filters.max_bathrooms) {
      where.number_of_bathrooms = {};
      if (filters.min_bathrooms) where.number_of_bathrooms.gte = filters.min_bathrooms;
      if (filters.max_bathrooms) where.number_of_bathrooms.lte = filters.max_bathrooms;
    }

    if (filters.min_size || filters.max_size) {
      where.size_square_meters = {};
      if (filters.min_size) where.size_square_meters.gte = filters.min_size;
      if (filters.max_size) where.size_square_meters.lte = filters.max_size;
    }
  ...
  async updateUnitStatus(unitId: string, status: string, user: JWTClaims): Promise<void> {
    // First check if unit exists and user has access
  ...
  async assignTenant(req: AssignTenantRequest, user: JWTClaims): Promise<void> {
    // First check if unit exists and user has access
  ...
  async releaseTenant(unitId: string, user: JWTClaims): Promise<void> {
    // First check if unit exists and user has access
  ...
  async searchAvailableUnits(filters: UnitFilters): Promise<any> {
  ...
  private hasUnitAccess(unit: any, user: JWTClaims): boolean {
    // Super admin has access to all units
    if (user.role === 'super_admin') return true;

    // Company scoping - user can only access units from their company
    if (user.company_id && unit.company_id === user.company_id) return true;

    // Property owner can access units in their properties
    if (unit.property && unit.property.owner_id === user.user_id) return true;

    // Agency admin can access units from properties in their agency
    if (user.role === 'agency_admin' && user.agency_id && unit.property && unit.property.agency_id === user.agency_id) return true;

    // Tenant can access their own unit
    if (user.role === 'tenant' && unit.current_tenant_id === user.user_id) return true;
    ...
    if (user.role === 'caretaker' && user.company_id && unit.property && unit.property.company_id === user.company_id) return true;
