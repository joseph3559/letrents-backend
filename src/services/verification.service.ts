...
// QRCode will be dynamically imported to handle ESM/CJS compatibility
// This avoids build-time errors if the package isn't installed yet

export type DocumentType = 'receipt' | 'invoice' | 'refund';
...
export class VerificationService {
   ...
   */
  buildVerificationUrl(documentType: DocumentType, documentNumber: string, token: string): string {
   ...
   */
  async verifyReceipt(receiptNumber: string, token: string): Promise<VerificationResult> {
   ...
   * Verify an invoice
   */
  async verifyInvoice(invoiceNumber: string, token: string): Promise<VerificationResult> {
   ...
   */
  async verifyRefund(receiptNumber: string, token: string): Promise<VerificationResult> {
    const payment = await this.prisma.payment.findUnique({
      where: { receipt_number: receiptNumber },
      include: {
        company: true,
   ...
   */
  async ensurePaymentVerificationToken(paymentId: string): Promise<{ token: string; qrUrl: string }> {
    const payment = await this.prisma.payment.findUnique({
      where: { id: paymentId },
    ...
    let token: string = payment.verification_token || '';
    if (!token) {
    ...
    const qrUrl: string = payment.qr_url || this.buildVerificationUrl('receipt', payment.receipt_number, token);
    if (!payment.qr_url) {
      await this.prisma.payment.update({
        where: { id: paymentId },
        data: { qr_url: qrUrl },
   ...
   */
  async ensureInvoiceVerificationToken(invoiceId: string): Promise<{ token: string; qrUrl: string }> {
    const invoice = await this.prisma.invoice.findUnique({
      where: { id: invoiceId },
    ...
    let token: string = invoice.verification_token || '';
    if (!token) {
      token = this.generateVerificationToken();
    ...
    const qrUrl: string = invoice.qr_url || this.buildVerificationUrl('invoice', invoice.invoice_number, token);
    if (!invoice.qr_url) {
      await this.prisma.invoice.update({
        where: { id: invoiceId },
        data: { qr_url: qrUrl },
