...
export interface TenantFilters {
  property_id?: string;
  property_ids?: string[]; // For super-admin filtering by multiple properties
  unit_id?: string;
  status?: string;
  search_query?: string;
  sort_by?: string;
...
export interface CreateTenantRequest {
  ...
  // Direct emergency contact fields (for compatibility)
...
This agreement is subject to the laws of Kenya and any disputes shall be resolved through arbitration.
  ...
  async checkDeletable(id: string, user: JWTClaims): Promise<{
    canDelete: boolean;
    reason?: string;
    details: {
      status: string;
  ...
  async deleteTenant(id: string, user: JWTClaims): Promise<void> {
    ...
    if (allLeases.length > 0) {
      ...
      throw new Error(
        `Cannot delete tenant with existing lease records (${activeCount} active, ${terminatedCount} terminated). ` +
  ...
  async listTenants(filters: TenantFilters, user: JWTClaims): Promise<any> {
    ...
    if (user.role === 'super_admin') {
      ...
      if (filters.property_ids && filters.property_ids.length > 0) {
        ...
        const directTenantIds = units.map(u => u.current_tenant_id).filter(id => id !== null) as string[];
    ...
    const invoicesByTenant = invoices.reduce((acc: any, invoice) => {
    ...
    const paymentsByTenant = pendingPayments.reduce((acc: any, payment) => {
    ...
    const calculatePaymentInfo = (tenantId: string) => {
      ...
      const invoiceBalance = tenantInvoices.reduce((sum: number, inv: any) => 
        sum + Number(inv.total_amount || 0), 0
      );

      // Separate pending and approved/completed payments
      const pendingPaymentsOnly = tenantPayments.filter((p: any) => p.status === 'pending');
      const approvedPayments = tenantPayments.filter((p: any) => 
        p.status === 'approved' || p.status === 'completed'
      );

      // Calculate pending payment amount (these are awaiting approval)
      const pendingPaymentAmount = pendingPaymentsOnly.reduce((sum: number, payment: any) => 
        sum + Number(payment.amount || 0), 0
      );

      // Calculate approved payment amount (these should REDUCE the balance)
      const approvedPaymentAmount = approvedPayments.reduce((sum: number, payment: any) => 
        sum + Number(payment.amount || 0), 0
      );
      ...
      // Determine payment status
      const now = new Date();
      const hasOverdueInvoice = tenantInvoices.some((inv: any) => 
        inv.status === 'overdue' || new Date(inv.due_date) < now
      );
   ...
   */
  async cleanupTerminatedTenants(): Promise<{ deleted: number; skipped: number; tenants: string[]; skippedTenants: string[] }> {
...
If you have any questions, please contact your property management team.
  ...
  ): Promise<{ activities: any[], hasMore: boolean, total: number }> {
  ...
  async getTenantMaintenance(tenantId: string, user: JWTClaims): Promise<any[]> {
    ...
    return maintenanceRequests.map((req: any) => ({
      id: req.id,
      title: req.title,
  ...
  async getTenantPerformance(tenantId: string, user: JWTClaims): Promise<any> {
    ...
    const onTimePayments = paidInvoices.filter((inv: any) => {
      const payment = payments.find((p: any) => p.invoice_id === inv.id);
      if (!payment || !payment.payment_date || !inv.due_date) return false;
    ...
    let rating = 5.0;
    if (onTimePercentage < 60) rating -= 2;
    else if (onTimePercentage < 80) rating -= 1;
    else if (onTimePercentage < 90) rating -= 0.5;

    if (maintenanceRequests.length > 10) rating -= 0.5;
    else if (maintenanceRequests.length > 5) rating -= 0.25;
    ...
    const propertyCare = maintenanceRequests.filter((m: any) => 
      m.category === 'damage' || m.priority === 'high'
    ).length <= 1 ? 'Very Good' : 'Good';
