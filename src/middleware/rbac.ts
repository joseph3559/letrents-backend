...
const permissions: Record<UserRole, Record<string, string[]>> = {
	...
	agency_admin: {
		...
		notifications: ['read', 'update', 'delete', 'bulk'],
...
export const rbacResource = (resource: string, action: string) => (req: Request, res: Response, next: NextFunction) => {
	...
	if (!allowed) return res.status(403).json({ success: false, message: `Role '${user.role}' does not have permission to '${action}' on '${resource}'` });
