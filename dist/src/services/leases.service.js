...
export class LeasesService {
    ...
    async createLease(req, user, retryCount = 0) {
    ...
    async updateLease(id, req, user) {
        ...
        const lease = await this.prisma.lease.update({
            where: { id },
            data: {
                ...(req.lease_type && { lease_type: req.lease_type }),
    ...
    async terminateLease(id, terminationReason, user) {
        // First check if lease exists and user has access
        const existingLease = await this.getLease(id, user);
        // Check permissions
        if (!['super_admin', 'agency_admin', 'landlord'].includes(user.role)) {
    ...
    async createLeasesForExistingTenants(user) {
        ...
        const unitsWithoutLeases = unitsWithTenantsWithoutLeases.filter(unit => unit.leases.length === 0);
        ...
        for (const unit of unitsWithoutLeases) {
            try {
                ...
                const leaseData = {
                    ...
                    lease_type: unit.lease_type || 'fixed_term',
    ...
    async createLeaseForExistingTenant(req, user) {
