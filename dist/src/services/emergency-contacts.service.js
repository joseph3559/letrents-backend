...
export class EmergencyContactsService {
     ...
     */
    async getEmergencyContacts(user, filters) {
     ...
     */
    async getEmergencyContact(contactId, user) {
     ...
     */
    async createEmergencyContact(req, user) {
        // Check permissions
     ...
     */
    async updateEmergencyContact(contactId, req, user) {
        // Check permissions
        if (!['super_admin', 'agency_admin', 'landlord'].includes(user.role)) {
        ...
        if (req.email !== undefined)
            updateData.email = req.email || null;
        if (req.available24_7 !== undefined)
            updateData.available24_7 = req.available24_7;
        if (req.response_time !== undefined)
            updateData.response_time = req.response_time || null;
        if (req.specialties !== undefined)
            updateData.specialties = req.specialties;
        if (req.property_ids !== undefined)
            updateData.property_ids = req.property_ids;
        if (req.is_local !== undefined)
            updateData.is_local = req.is_local;
        if (req.verified !== undefined)
            updateData.verified = req.verified;
        if (req.agent_assigned !== undefined)
            updateData.agent_assigned = req.agent_assigned || null;
     ...
     */
    async deleteEmergencyContact(contactId, user) {
        // Check permissions
        if (!['super_admin', 'agency_admin', 'landlord'].includes(user.role)) {
