...
export const messagingService = {
     ...
     */
    async getOrCreateConversation(user, participantIds, isGroup = false, groupName) {
        // Validate participants
        const allParticipants = [user.user_id, ...participantIds].filter((id, index, self) => self.indexOf(id) === index);
        ...
        if (!isGroup && allParticipants.length === 2) {
            ...
            if (existingConversation) {
                ...
                const matchesAll = existingConversation.participants.every(p => allParticipants.includes(p.user_id));
    ...
    async createMessage(user, data) {
        ...
        if (!conversationId) {
            ...
            const conversation = await this.getOrCreateConversation(user, data.recipientIds, false);
            if (!conversation) {
     ...
     */
    async getMessages(user, conversationId, limit = 50, offset = 0, before) {
        ...
        if (unreadMessageIds.length > 0) {
            ...
            for (const message of messages.filter(m => unreadMessageIds.includes(m.id))) {
                try {
                    await supabaseRealtimeService.publishReadStatus(message.sender_id, message.id, user.user_id, new Date());
                }
                catch (error) {
     ...
     * Update message (edit)
     */
    async updateMessage(user, messageId, data) {
     ...
     * Delete message
     */
    async deleteMessage(user, messageId, deleteForEveryone = false) {
        const message = await prisma.message.findUnique({
            where: { id: messageId },
            include: {
                recipients: true,
     ...
     */
    async addReaction(user, messageId, reactionType) {
        const message = await prisma.message.findUnique({
            where: { id: messageId },
        });
        if (!message) {
     ...
     */
    async removeReaction(user, messageId, reactionType) {
        await prisma.$executeRaw `
     ...
     * Pin message
     */
    async pinMessage(user, conversationId, messageId, note) {
        // Verify user is participant
        const participant = await prisma.conversationParticipant.findFirst({
            where: {
                conversation_id: conversationId,
     ...
     * Unpin message
     */
    async unpinMessage(user, conversationId, messageId) {
        await prisma.$executeRaw `
     ...
     */
    async getConversations(user, limit = 50, offset = 0) {
     ...
     */
    async updatePresence(user, status, message) {
     ...
     */
    async updateTypingIndicator(user, conversationId, isTyping) {
        // Verify user is participant
        const participant = await prisma.conversationParticipant.findFirst({
            where: {
                conversation_id: conversationId,
