 ...
 */
async function generateStaffNumber(role, companyId) {
...
export const staffService = {
     ...
     */
    async getStaffMembers(user, role, filters = {}) {
    ...
    async createStaffMember(user, role, staffData) {
        ...
        const createData = {
            ...
            role: role,
            status: passwordHash ? 'active' : 'pending', // Set to pending if no password
        ...
        if (staffData.assigned_properties && Array.isArray(staffData.assigned_properties) && staffData.assigned_properties.length > 0) {
            const propertyAssignments = staffData.assigned_properties.map((propertyId, index) => ({
     ...
     */
    async getStaffMember(user, staffId, role) {
        const whereClause = buildWhereClause(user, {
            id: staffId,
            ...(role ? { role: role } : {}),
        }, 'user');
     ...
     */
    async updateStaffMember(user, staffId, updateData, role) {
        ...
        try {
            ...
            if (updateData.assigned_properties) {
                ...
                if (Array.isArray(updateData.assigned_properties) && updateData.assigned_properties.length > 0) {
                    await prisma.staffPropertyAssignment.createMany({
                        data: updateData.assigned_properties.map((propertyId) => ({
                            staff_id: staffId,
                            property_id: propertyId,
                            status: 'active',
     ...
     */
    async deleteStaffMember(user, staffId, role) {
     ...
     */
    async inviteStaffMember(user, staffId, role) {
        // Check if staff member exists and user has access
...
The LetRents Team`,
     ...
     */
    async resetPassword(user, staffId, role) {
        // Check if staff member exists and user has access
        ...
        try {
            const { emailService } = await import('./email.service.js');
            const emailResult = await emailService.sendPasswordResetEmail(staffMember.email, resetUrl, `${staffMember.first_name} ${staffMember.last_name}`);
            if (!emailResult.success) {
...
export const careteakersService = {
    getCaretakers: async (user, filters) => {
        ...
        if (propertyIds && Array.isArray(propertyIds) && propertyIds.length > 0) {
            ...
            const staffIds = staffWithAssignments.map((a) => a.staff_id);
    ...
    createCaretaker: (user, data) => staffService.createStaffMember(user, data.role || 'caretaker', data), // Use role from data, fallback to 'caretaker'
    getCaretaker: (user, id) => staffService.getStaffMember(user, id), // No role filter - fetch any staff member
    updateCaretaker: (user, id, data) => staffService.updateStaffMember(user, id, data, 'caretaker'),
    deleteCaretaker: (user, id) => staffService.deleteStaffMember(user, id), // Don't pass role - find by ID only
    inviteCaretaker: (user, id) => staffService.inviteStaffMember(user, id), // Don't pass role - let it auto-detect
    resetPassword: (user, id) => staffService.resetPassword(user, id, 'caretaker'),
};
