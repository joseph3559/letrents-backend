 ...
 */
async function sendFCMNotification(token, platform, notification) {
    try {
...
export const pushNotificationService = {
     ...
     */
    async registerToken(userId, token, platform, deviceId, deviceInfo) {
        try {
    ...
    async sendToUser(userId, notification) {
        try {
            ...
            const shouldSend = await this.shouldSendNotification(userId, notification.category || 'general', notification.priority === 'high' ? 'high' : notification.priority || 'medium');
            if (!shouldSend) {
     ...
     */
    async sendToTargets(targets, notification) {
        let userIds = [];
     ...
     */
    async shouldSendNotification(userId, category, priority = 'medium') {
        try {
     ...
     */
    async logDelivery(notificationId, userId, channel, status, metadata) {
        try {
            await prisma.$executeRaw `
