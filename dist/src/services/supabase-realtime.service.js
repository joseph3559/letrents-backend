...
class SupabaseRealtimeService {
     ...
     */
    async publishMessageUpdate(message) {
        if (!this.supabase)
            return false;
        try {
            if (message.conversation_id) {
     ...
     */
    async publishMessageDeletion(messageId, conversationId, deletedForEveryone) {
        if (!this.supabase)
            return false;
        try {
            if (conversationId) {
     ...
     */
    async publishReaction(messageId, conversationId, reactionType, userId, added) {
        if (!this.supabase)
            return false;
        try {
            if (conversationId) {
     ...
     */
    async publishPresenceUpdate(userId, status, message) {
        if (!this.supabase)
            return false;
        try {
     ...
     */
    async publishTypingIndicator(conversationId, userId, recipientIds, isTyping) {
        if (!this.supabase)
            return false;
        try {
     ...
     */
    async publishNotificationCount(recipientId, unreadCount) {
        if (!this.supabase)
            return false;
        try {
     ...
     */
    async publishNotificationRead(notificationId, recipientId, senderId) {
        if (!this.supabase)
            return false;
        try {
            // Notify recipient
     ...
     */
    async publishConversationUpdate(conversationId, participants, data) {
        if (!this.supabase)
            return false;
        try {
            const channel = this.getOrCreateChannel(`conversation:${conversationId}`);
