...
export const PAYSTACK_CHANNEL_MAP = {
 ...
 */
export function toShortReference(ref) {
 ...
 */
export function toShortTransactionId(id) {
...
export function formatPaymentMethodDisplay(payment) {
    ...
    if (method !== 'online') {
        if (method === 'cash')
            return 'Cash';
        if (method === 'mpesa' || method === 'mobile_money')
            return 'Mobile Money (M-Pesa, Airtel Money)';
    ...
    if (!first || typeof first !== 'object')
        return 'Online';
    const obj = first;
    let ch = obj.channel || obj.channel_display;
    if (!ch && obj.gateway_response && typeof obj.gateway_response === 'object') {
        const gw = obj.gateway_response;
        ch = gw.channel || (gw.data && typeof gw.data === 'object' ? gw.data.channel : '');
    }
 ...
 */
export function getChannelDisplay(channel, authorization) {
    if (!channel)
        return 'Online';
    const k = channel.toLowerCase().replace(/-/g, '_');
    if (PAYSTACK_CHANNEL_MAP[k])
        return PAYSTACK_CHANNEL_MAP[k];
