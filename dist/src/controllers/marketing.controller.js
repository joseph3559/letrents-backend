...
export const getJobPosting = async (req, res) => {
    try {
        const { id } = req.params;
        const job = await prisma.$queryRawUnsafe(`SELECT 
        id, title, department, location, employment_type, 
        description, requirements, responsibilities, benefits, salary_range,
        status, application_deadline, posted_by, created_at, updated_at,
        published_at, views_count, applications_count
      ...
      WHERE id = $1::uuid`, id);
        ...
        await prisma.$executeRawUnsafe(`UPDATE job_postings 
...
export const createJobPosting = async (req, res) => {
    try {
        const user = req.user;
        const { title, department, location, employment_type, description, requirements, responsibilities, benefits, salary_range, application_deadline, status = 'draft' } = req.body;
...
export const updateJobPosting = async (req, res) => {
    try {
        const { id } = req.params;
        const { title, department, location, employment_type, description, requirements, responsibilities, benefits, salary_range, application_deadline, status } = req.body;
        // Check if job exists
        const existing = await prisma.$queryRawUnsafe(`SELECT id FROM job_postings WHERE id = $1::uuid`, id);
...
export const getJobApplication = async (req, res) => {
    try {
        const { id } = req.params;
        const application = await prisma.$queryRawUnsafe(`SELECT 
        id, job_posting_id, first_name, last_name, email, phone_number,
        cover_letter, resume_url, linkedin_url, portfolio_url,
        years_of_experience, current_position, current_company,
        expected_salary, availability_date, status, notes,
...
export const createJobApplication = async (req, res) => {
    try {
        const { job_posting_id, first_name, last_name, email, phone_number, cover_letter, resume_url, linkedin_url, portfolio_url, years_of_experience, current_position, current_company, expected_salary, availability_date } = req.body;
...
export const updateJobApplication = async (req, res) => {
    try {
        ...
        const existing = await prisma.$queryRawUnsafe(`SELECT id FROM job_applications WHERE id = $1::uuid`, id);
        if (!existing || existing.length === 0) {
            return res.status(404).json({
                success: false,
                message: 'Job application not found'
...
export const createFAQ = async (req, res) => {
    try {
        ...
        const faq = await prisma.$queryRawUnsafe(`INSERT INTO faqs (question, answer, category, order_index, is_published, created_by)
      VALUES ($1, $2, $3, $4, $5, $6::uuid)
      RETURNING *`, question, answer, category || null, order_index || 0, is_published, user.id);
        res.status(201).json({
            success: true,
            data: faq[0],
...
export const deleteFAQ = async (req, res) => {
    try {
        const { id } = req.params;
        await prisma.$executeRawUnsafe(`DELETE FROM faqs WHERE id = $1::uuid`, id);
        res.json({
            success: true,
