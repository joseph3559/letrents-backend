...
export const getTenantPendingPayables = async (req, res) => {
    try {
        const user = req.user;
        if (user.role !== 'tenant') {
            return res.status(403).json({
        ...
        const invoicesByType = pendingInvoices.reduce((acc, invoice) => {
...
export const updateTenantProfile = async (req, res) => {
    try {
        ...
        const { first_name, last_name, phone_number, id_number, emergency_contact_name, emergency_contact_phone, emergency_contact_relationship, preferred_communication_method } = req.body;
...
export const uploadTenantProfilePicture = async (req, res) => {
    try {
        const user = req.user;
        if (user.role !== 'tenant') {
            return res.status(403).json({
        ...
        const uploadResult = await imagekitService.uploadFile(req.file.buffer, fileName, 'tenant-profiles');
...
export const submitLeaseEditRequest = async (req, res) => {
    try {
        const user = req.user;
        if (user.role !== 'tenant') {
            return res.status(403).json({
        ...
        const { lease_id, modification_type, details, proposed_effective_date, reason } = req.body;
...
export const getTenantLeaseModifications = async (req, res) => {
    try {
        const user = req.user;
...
export const getTenantAllModifications = async (req, res) => {
    try {
        const user = req.user;
        if (user.role !== 'tenant') {
            return res.status(403).json({
        ...
        const modificationType = req.query.type;
        // Dynamically import the service
        const { LeaseModificationsService } = await import('../services/lease-modifications.service.js');
        const modificationsService = new LeaseModificationsService();
...
export const acknowledgeTenantLeaseModification = async (req, res) => {
    try {
        const user = req.user;
...
export const getTenantUnacknowledgedModifications = async (req, res) => {
    try {
        const user = req.user;
        if (user.role !== 'tenant') {
            return res.status(403).json({
...
export const getTenantLeaseModificationStats = async (req, res) => {
    try {
        const user = req.user;
        const { leaseId } = req.params;
        if (user.role !== 'tenant') {
...
export const processTenantPayment = async (req, res) => {
    try {
        const user = req.user;
        if (user.role !== 'tenant') {
...
export const cancelTenantPendingPayment = async (req, res) => {
    try {
        const user = req.user;
...
export const cleanupDuplicatePayment = async (req, res) => {
    try {
        const user = req.user;
        const { id: paymentId } = req.params;
        if (user.role !== 'tenant') {
