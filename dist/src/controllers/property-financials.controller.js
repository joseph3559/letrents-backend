...
export const propertyFinancialsController = {
    ...
    getPropertyPayments: async (req, res) => {
        try {
            const user = req.user;
    ...
    getPropertyUtilityBills: async (req, res) => {
        try {
            const user = req.user;
            const { propertyId } = req.params;
            const { limit = 50, offset = 0 } = req.query;
    ...
    getPropertyOutstandingBalance: async (req, res) => {
        try {
            const user = req.user;
            const { propertyId } = req.params;
            // Verify property access
            ...
            const overdueCount = invoices.filter(invoice => new Date(invoice.due_date) < new Date() &&
                ['draft', 'sent', 'overdue'].includes(invoice.status)).length;
    ...
    getPropertyMonthlyExpenses: async (req, res) => {
        try {
            const user = req.user;
            const { propertyId } = req.params;
            ...
            // Calculate totals
            const maintenanceTotal = maintenanceExpenses.reduce((total, expense) => total + Number(expense.actual_cost || 0), 0);
            const utilityTotal = utilityExpenses.reduce((total, expense) => total + Number(expense.amount), 0);
    ...
    getPropertyMpesaTransactions: async (req, res) => {
        try {
            const user = req.user;
            const { propertyId } = req.params;
            const { limit = 50, offset = 0 } = req.query;
