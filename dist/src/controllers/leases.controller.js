import { LeasesService } from '../services/leases.service.js';
...
export class LeasesController {
    ...
    renewLease = async (req, res) => {
        try {
            ...
            const missingFields = requiredFields.filter(field => !renewalData[field]);
            if (missingFields.length > 0) {
    ...
    createLeasesForExistingTenants = async (req, res) => {
        try {
            const user = req.user;
