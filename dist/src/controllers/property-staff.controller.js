...
export const propertyStaffController = {
    ...
    getPropertyStaff: async (req, res) => {
        try {
            const user = req.user;
    ...
    assignStaffToProperty: async (req, res) => {
        try {
            const user = req.user;
            const { propertyId } = req.params;
    ...
    // Update staff member
    updatePropertyStaff: async (req, res) => {
        try {
            const user = req.user;
    ...
    removeStaffFromProperty: async (req, res) => {
        try {
            const user = req.user;
            const { propertyId, staffId } = req.params;
            // Find the property first to ensure user has access
...
// Helper functions
function generateStaffNumber(role, createdAt) {
    const rolePrefix = {
        'caretaker': 'CT',
        'agent': 'AG',
        'security': 'SC',
...
function getSpecializationByRole(role) {
    switch (role) {
        case 'caretaker':
...
function getOffDaysByRole(role) {
    switch (role) {
        case 'caretaker':
            return ['Sunday'];
        case 'agent':
