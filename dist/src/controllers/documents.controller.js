...
export const uploadTenantDocuments = async (req, res) => {
    try {
        const user = req.user;
        ...
        const uploadedDocuments = await Promise.all(files.map(async (file, index) => {
            ...
            const uploadResult = await imagekitService.uploadFile(file.buffer, fileName, `tenants/${tenantId}/documents`);
...
export const uploadUnitDocuments = async (req, res) => {
    try {
        const user = req.user;
        ...
        const uploadedDocuments = await Promise.all(files.map(async (file, index) => {
            ...
            const uploadResult = await imagekitService.uploadFile(file.buffer, fileName, `units/${unitId}/documents`);
            return {
                id: randomUUID(),
                name: file.originalname,
                type: file.mimetype,
        ...
        const currentDocuments = Array.isArray(unit.documents)
            ? unit.documents
            : [];
...
export const getUnitDocuments = async (req, res) => {
    try {
        const user = req.user;
        const { id: unitId } = req.params;
        if (!unitId) {
        ...
        const unit = await unitsService.getUnit(unitId, user);
        const documents = Array.isArray(unit.documents)
            ? unit.documents
            : [];
