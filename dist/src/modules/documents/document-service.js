...
import { toShortReference } from '../../utils/format-payment-display.js';
...
export class DocumentService {
     ...
     */
    clearTemplateCache(documentType, version) {
    ...
    async renderDocument(documentType, version, context, cacheKey) {
    ...
    templateChecksum(html, css) {
        return crypto.createHash('sha256').update(html).update('\n/*css*/\n').update(css).digest('hex');
    }
    async upsertTemplateRecord(documentType, version, html, css) {
    ...
    async getLatestSnapshot(documentType, entityType, entityId) {
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const prismaAny = this.prisma;
        if (!prismaAny.documentSnapshot)
            return null;
    ...
    async createSnapshotIfMissing(documentType, entityType, entityId, documentNumber, templateVersion, context, user) {
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const prismaAny = this.prisma;
        if (!prismaAny.documentSnapshot)
            return;
    ...
    async createSnapshotRevision(documentType, entityType, entityId, documentNumber, templateVersion, context, user) {
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const prismaAny = this.prisma;
        if (!prismaAny.documentSnapshot)
            return;
    ...
    buildInvoiceContext(invoice, qrCodeSvg = '', qrUrl = '', verificationToken = '') {
        ...
        return {
            ...
            company: {
                ...
                metaHtml: (() => {
                    ...
                    const parts = [];
                    if (address)
                        parts.push(`<div class="company-meta">${escapeAttr(address)}</div>`);
                    if (email || phone) {
            ...
            sections: {
                ...
                // Consolidated two-column layout: BILL TO
                billToRows: this.buildCompactInfoRows([
                    { label: 'Bill To', value: invoice.recipient ? `${invoice.recipient.first_name || ''} ${invoice.recipient.last_name || ''}`.trim() || 'N/A' : 'N/A' },
                    ...(invoice.property?.name ? [{ label: 'Property', value: invoice.property.name }] : []),
                    ...(invoice.unit?.unit_number ? [{ label: 'Unit', value: invoice.unit.unit_number }] : []),
                ]),
                // Consolidated two-column layout: Invoice Details
                invoiceInfoRows: this.buildCompactInfoRows([
                ...
                billToRowsLegacy: this.buildInfoRows([
                ...
                invoiceInfoRowsLegacy: this.buildInfoRows([
                    { label: 'Issue Date', value: formatDate(invoice.issue_date) },
                    { label: 'Due Date', value: formatDate(invoice.due_date) },
                    ...(invoice.paid_date ? [{ label: 'Paid Date', value: formatDate(invoice.paid_date) }] : []),
                ]),
        ...
        ` : '',
                ...
                footerMeta: (() => {
                    ...
                    if (invoice.payment_reference) {
                        parts.push(`Ref: ${invoice.payment_reference}`);
                    }
                    parts.push(`Powered by LetRents`);
                    return parts.map((part, i) => i === 0 ? `<span>${part}</span>` : `<span class="separator">•</span><span>${part}</span>`).join('');
                })(),
            },
        };
    }
    buildPaymentReceiptContext(payment, invoice = null, qrCodeSvg = '', qrUrl = '', verificationToken = '') {
        ...
        if (invoice?.line_items && invoice.line_items.length > 0) {
            ...
            breakdownTable = this.buildPaymentBreakdownTable([{ description, amount: formatMoney(toNumber(payment.amount), currency) }], formatMoney(toNumber(payment.amount), currency), currency);
        }
        return {
            meta: {
            ...
            sections: {
                detailsRows: buildKeyValueRows([
                    ...
                    ...(payment.reference_number ? [{ label: 'Reference', value: toShortReference(payment.reference_number) }] : []),
                ...
                breakdownTable,
                // Consolidated two-column layout: RECEIVED FROM
                payerPropertyRows: this.buildCompactInfoRows([
                    { label: 'Received From', value: payment.tenant ? `${payment.tenant.first_name || ''} ${payment.tenant.last_name || ''}`.trim() || 'N/A' : (payment.received_from || 'N/A') },
                    ...(payment.property?.name ? [{ label: 'Property', value: payment.property.name }] : []),
                    ...(payment.unit?.unit_number ? [{ label: 'Unit', value: payment.unit.unit_number }] : []),
                    ...(payment.payment_period ? [{ label: 'Period', value: payment.payment_period }] : []),
                ]),
                // Consolidated two-column layout: Payment Details
                paymentDetailsRows: this.buildCompactInfoRows([
                    { label: 'Date', value: formatDateTime(payment.payment_date) },
                    { label: 'Method', value: String(payment.payment_method || 'N/A').toUpperCase() },
    ...
    buildPaymentBreakdownTable(items, total, currency) {
    ...
    buildRefundReceiptContext(payment, qrCodeSvg = '', qrUrl = '', verificationToken = '') {
        const currency = payment.currency || 'KES';
        return {
            meta: {
     ...
     */
    async recordInvoiceSnapshot(invoiceId, user, version = 1) {
    ...
    async recordPaymentReceiptSnapshot(paymentId, user, version = 1) {
    ...
    async recordRefundReceiptSnapshot(paymentId, user, version = 1) {
        const payment = await this.prisma.payment.findUnique({
            where: { id: paymentId },
            include: { company: true, tenant: true, property: true, unit: true },
        });
    ...
    async recordLeaseSnapshot(leaseId, user, version = 1) {
    ...
    async getInvoicePdf(invoiceId, user, version = 1) {
    ...
    async getPaymentReceiptPdf(paymentId, user, version = 1) {
        const payment = await this.prisma.payment.findUnique({
            where: { id: paymentId },
            include: {
                company: true,
        ...
        console.log('Payment Receipt Context Debug:', {
            ...
            hasQrCodeSvg: !!context.sections?.qrCodeSvg,
            qrCodeSvgLength: context.sections?.qrCodeSvg?.length || 0,
            hasVerificationToken: !!context.sections?.verificationToken,
            payerInfoRowsLength: context.sections?.payerInfoRows?.length || 0,
            paymentInfoRowsLength: context.sections?.paymentInfoRows?.length || 0,
            paidToRowsLength: context.sections?.paidToRows?.length || 0,
            hasBreakdownTable: !!context.sections?.breakdownTable,
        });
    ...
    async getRefundReceiptPdf(paymentId, user, version = 1) {
        const payment = await this.prisma.payment.findUnique({
            where: { id: paymentId },
            include: {
                company: true,
        ...
        if (payment.status !== 'refunded')
            throw new Error('payment is not refunded');
        if (user.role !== 'super_admin' && user.company_id && payment.company_id !== user.company_id) {
            if (!(user.role === 'tenant' && payment.tenant_id === user.user_id)) {
    ...
    async getLeasePdf(leaseId, user, version = 1) {
        const lease = await this.prisma.lease.findUnique({
            where: { id: leaseId },
            include: {
                company: true,
    ...
    async getTenantStatementPdf(tenantId, startIso, endIso, user, version = 1) {
        ...
        const currency = tenant.company?.settings && tenant.company.settings?.default_currency
            ? String(tenant.company.settings.default_currency)
            : 'KES';
    ...
    async getReportPdf(reportType, title, rows, summary, user, version = 1) {
